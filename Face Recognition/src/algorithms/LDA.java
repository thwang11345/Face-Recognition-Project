...
public class LDA {
	...
	/**
	 * Given features, scales features using z-score normalization. Returns null if norm < EPSILON
	 * @param x original features
	 * @return scaled features or null if norm < EPSILON
	 */
	...
	/*
	 ********************************************************************
	 * PCA (Principal Component Analysis) Procedure: Calculate eigenvalue of A
	 * At(nxn), instead of At A(dxd), usually n<<d, wi is eigenvector of At A, the
	 * 
	 * vi=At wi (dx1) is eigenvector of At A
	 * 
	 * 1. M; -- Mean, dx1 2. A -- A nxd, M,X dx1 A[k] = Xt[k] - M; k=1,2,..., n --
	 * At = transpose of U; -- At dxn 3 Cov=1/n At A -- 4. Call jacobi(Cov, d,
	 * Lambda, W, &IterationNo)-- W is dxd Call eigensort(Lambda, W); -- 5. Find
	 * sum_lambda = sum of all Lambda -- denominator percent = 0 m = 0; partial_sum
	 * = 0; while (percent < Mef_PerCentage) { partial_sum += lambda[m]; percent =
	 * partial_sum/sum_lambda; m++; } 6. Ht first m column of W -- Ht dxm 7. Yt = A
	 * Ht -- Yt nxm
	 ********************************************************************
	 */
	/**
	 * Calculate the projection of n samples onto m eigenvectors, reducing the dimensions from n x d to n x m
	 * where m eigenvalues reach PCA_PERCENTAGE threshold. The projection represents the same data in m dimensions where m < d. 
	 * @param Xt n x d representation of all the samples
	 * @param n the number of samples
	 * @param d the length/features of a sample (128 x 128)
	 * @return the projection of n samples onto m eigenvectors
	 */
	static public PCAMap PCA(double[] Xt, int n, int d) {
		Integer m;
		double[] Ht; // m eigenvectors
		double[] Yt; // n by m projection
		double[] M; // mean
		...
		// Use the property of symmetry b/c covarience is always symmetric
		for (j = 0; j < n; j++) {
			for (i = 0; i < j; i++)
				Cov[j * n + i] = Cov[i * n + j];
		}
		...
		Lambda = new double[n]; // eigen values
		e = new double[n];
		QRSolver.s_tred2(Cov, n, Lambda, e); // 
		QRSolver.s_tqli(Lambda, e, n, Cov); // 
		QRSolver.s_eigensort(Lambda, Cov, n); // Cov becomes eigen vectors

		// Find how many eigen vectors to use
		sum_lambda = 0;
		for (j = 0; j < n; j++)
			sum_lambda += Lambda[j];
		m = 0;
		...
		// projecting samples to eigen vectors
		Ht = new double[d * m];
		for (j = 0; j < d; j++) {
			j1 = j * m;
			j2 = j * n;
		...
		// Calculate Yt, Yt nxm Yt = A*Ht which is the projection
	...
	/*
	 ********************************************************************
	 * LEARN_MDF: 1. Calculate within-class scatter matrix Sw -- note Sw is mxm. 2.
	 * d[i] = m[i] - M, i=0..c -- note mx1 Sb = 0; -- note mxm for (i=0; i<c; i++) {
	 * Sb += scalarmatrixmult( p[i], matrixmult(d[i], m, 1, trans(d[i], m, 1), 1,
	 * m))); -- from utils 3. SbInv = matrixinv(Sb, m, m); -- from NR p. 45 4. L =
	 * matrixmult(SwInv, m, m, Sb, m, m); -- from utils 5. From Eispack, p. 26: call
	 * balanc, elmhes, eltran, hqr2, balbak. These routines were written in FORTRAN
	 * and need to be converted to C. Note the NSWC code may have alternative
	 * solutions, but I have not found documentation for them yet. 6. Call
	 * sort(Lambda, W); -- from utils
	 ********************************************************************/
	static public LDAMap Learn(double[] set, int[] class_labels, int len) {
		...
		/********
		 * Finding the eigenvectors/eigenvalues of the matrix Sw-inverse * SB
		 * 
		 * 1. Compute H, Lambda: eigenvectors/eigenvalues of Sw. 2. Sort the
		 * eigensystem. 2.5.Add the median eigenvalue to Lambda 3. Set Lambda =
		 * 1/(sqrt(Lambda)). 4. Compute H = H Lambda. 5. Compute splitB = H-transpose *
		 * Sb * H 6. Compute U, sigma: eigenvectors/eigenvalues of splitB. 7. Sort the
		 * eigensystem. 8. Compute W = H U 9. Normalize the columns of W.
		 * 
		 * W is the MDF projection matrix. Note for now, W should be float because the
		 * other programs are expecting it to be.
		 *******/
